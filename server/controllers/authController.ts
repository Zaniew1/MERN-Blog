...
export const getMe = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { name, surname, avatar, email } = (await UserModel.findById(
      req.user?._id
    )) as UserSchemaType;
    res.status(200).json({
      status: "success",
...
export const createNewUser: RequestHandler = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    // validating data
    const { name, surname, email, password, confirmPassword } =
      req.body as NewUserType;
    if (!email || !password)
      return next(new AppError("There have to be email and password", 400));
    if (!name || !surname)
      return next(new AppError("There have to be name and surname", 400));
    if (
      !password.match(
        /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/
      )
      ...
      return next(
        new AppError(
          "Password needs to contain min. 8 letters, 1 big letter, 1 small letter and 1 special character",
          400
        )
      );
    }
...
export const loginUser: RequestHandler = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      return next(
        new AppError(
          "Password needs to contain min. 8 letters, 1 big, 1 small letter and 1 special character",
          400
        )
      );
    }
...
export const changePassword: RequestHandler = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { oldPass, newPass, confirmNewPass, email } =
    ...
    if (!isGoodPassword)
      return next(
        new AppError("Old password that you provided was incorrect", 400)
      );
...
export const editUser: RequestHandler = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id, name, surname, avatar } = req.body as EditUserType;
    if (!name || !surname)
      return next(new AppError("Imię i nazwisko jest wymagane", 400));
    if (name.length < 3 || surname.length < 3)
      return next(
        new AppError("Imię i nazwisko musi mieć przynajmniej 3 znaki", 400)
      );
    if (name.length > 12 || surname.length > 15)
      return next(
        new AppError("Imię musi mieć max 12 znaków a nazwisko max 15", 400)
      );
